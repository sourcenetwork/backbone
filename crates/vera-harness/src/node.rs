...
use crate::genesis;
use crate::identity::vera_address;
use crate::VeraPorts;
...
/// A running Vera single-node devnet.
///
...
impl VeraNode {
    ...
    pub async fn start(
        ...
        let funded_addresses: Vec<String> = identity_keys
            .iter()
            .map(|key| vera_address(key))
            .collect::<Result<_>>()
            .wrap_err("deriving Vera addresses from identity keys")?;

        let faucet_address = vera_address(TEST_ACCOUNT_HEX_KEY)?;
        ...
        .wrap_err("provisioning Vera genesis")?;
        ...
        let log_tracker =
            test_infra::LogTracker::start(stdout_path, "committed state", vera_patterns());

        let process = test_infra::ManagedProcess::spawn("vera", &binary, &args, &[], &log_dir)
...
fn vera_patterns() -> Vec<test_infra::NamedPattern> {
